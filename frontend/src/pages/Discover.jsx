...
// function enrichWritings(writingsData, commentsMap, likesMap) {
//     return writingsData.map(writing => ({
//         ...writing,
//         comments: commentsMap.get(writing._id) || [],
//         userLike: likesMap.get(writing._id) || null
//     }));
// }
...
//     const likesByWritingId = new Map();
//     writings?.likesInfo?.forEach((like) => {
//         likesByWritingId.set(like.writingId, like);
//     });

//     const commentsByWritingId = new Map();
//     writings?.commentsInfo?.forEach((comment) => {
//         if (!commentsByWritingId.has(comment.writingId)) {
//             commentsByWritingId.set(comment.writingId, []);
//         }
//         commentsByWritingId.get(comment.writingId).push(comment);
//     });

//     useEffect(() => {
//         if (writings?.data) {
//             // setWritingsData(writings.data);
//             const enriched = enrichWritings(writings.data, commentsByWritingId, likesByWritingId);
//             setWritingsData(enriched);
...
//                                                 key={writing._id}
//                                                 writing={writing}
//                                                 // userLike={likesByWritingId.get(writing._id)} // ✅ user-specific like info
//                                                 // userComments={commentsByWritingId.get(writing._id) || []} // ✅ user-specific comments
...
const Discover = () => {
        ...
        <div className="container mx-auto px-4 py-8 bg-background">
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="md:hidden space-y-4">
                    ...
                    {/* Popular Tags */}
                    <div>
                        <p className="text-sm font-medium text-muted-foreground mb-2 font-sans">Popular Tags</p>
                        <div className="flex flex-wrap gap-2">
                            {categories.map((tag) => (
                                <button
                                    key={tag}
                                    ...
                                    className={`cursor-pointer rounded-full px-3 py-1.5 transition-all duration-200 text-sm font-sans hover:scale-105 active:scale-95
            ${tag === filter.tag
                                            ? "bg-accent text-white font-semibold shadow-sm"
                                            : "bg-accent/60 text-accent-foreground hover:bg-accent/10 hover:text-accent border border-border"
                                        }
          `}
                                >
                                    {tag}
                ...
                <div className="w-full lg:w-80 space-y-6 lg:block hidden">
                    <div className="bg-card rounded-lg p-6 border border-border shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex justify-between items-start">
                            ...
                            <button
                                onClick={cleatAllFilter}
                                className="font-medium px-3 py-1 rounded-md bg-accent/40 text-accent-foreground hover:bg-accent hover:text-accent-foreground transition-colors text-sm font-sans"
                            >
                                Clear filters
                            </button>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-sm font-medium text-foreground mb-2 block font-sans">Search</label>
                                <div className="relative">
                                    <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                                    <input
                                        placeholder="Search stories, poems..."
                            ...
                            {/* Content Type */}
                            <div>
                                <label className="text-sm font-medium text-foreground mb-3 block font-sans">Content Type</label>
                                <div className="space-y-1">
                                    {contentTypes.map((type) => {
                                        ...
                                        return (
                                            <button
                                                key={type.id}
                                                className={`w-full rounded-lg p-3 flex items-center justify-start transition-all duration-200 font-sans text-sm active:scale-[0.98] ${filter.contentType === type.id
                            ...
                            {/* Popular Tags */}
                            <div>
                                <label className="text-sm font-medium text-foreground mb-3 block font-sans">Popular Tags</label>
                                <div className="flex flex-wrap gap-2">
                                    {categories.map((tag) => (
                                        <button
                                            key={tag}
  ...
  ${tag === filter.tag
                ...
                <div className="flex-1">
                    <div className="mb-6">
                        {/* <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-foreground mb-2 font-serif">
                            Discover Literary Content
                        </h1>
                        ...
                        {loading ? (
                            ...
                            <div className="mt-52">
                                <p className="text-center text-muted-foreground font-sans text-base">
                                    No writings available in this category. Please check back later!
                                </p>
                            </div>
                        ) : (
                            <>
