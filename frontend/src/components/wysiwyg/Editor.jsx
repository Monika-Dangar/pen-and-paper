...
const Editor = () => {
    ...
    const categories = [
        'Love', 'Faith', 'Loss', 'Youth', 'Fiction', 'Dreams', 'Nature', 'Sad', 'Spiritual',
        'Family', 'Friendship', 'Philosophy', 'Fantasy', 'Life', 'Inspirational', 'Freedom',
        'Hope', 'Grief', 'Mystery', 'Happiness'
    ];
    ...
    const handlePublish = async (e) => {
        ...
        if (!editorContent || editorContent.trim() === '' || editorContent === 'Please writer here.') {
            setContentError('Content is required');
            const editor = editorRef.current.editor;
            editor.opts.placeholderText = contentError ? 'Content is required' : 'Write your content here!';
        ...
        if (writingData && !isNewWriting) {
            ...
            if (editorContent !== writingData.content) postData.content = editorContent;
        ...
        <section>
            <form className="flex flex-col md:flex-row justify-between items-center space-x-3 space-y-4 md:space-y-0">
                <div className="w-full md:w-3/5">
                    ...
                    <input
                        ...
                        type="text"
                        name="title"
                        className={`px-5 py-2 w-full rounded-md border border-gray-300 shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${titleError ? 'border-red-500' : ''}`}
                ...
                <div className="w-full text-center md:w-2/5">
                    ...
                    <select
                        ...
                        name="category"
                        required
                        className={`px-5 py-2 rounded-md border border-gray-300 shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${categoryError ? 'border-red-500' : ''}`}
                        value={category}
                        onChange={(e) => setCategory(e.target.value)}
                    >
                        <option value="" disabled>{`${categoryError ? 'Category is required' : 'Select category'}`}</option>
                        {categories.map((cat) => (
            ...
            <div id="froala-editor" className="relative z-0 mt-4">
                <FroalaEditor
                    ref={editorRef}
                    config={{
                        placeholderText: contentError ? 'Content is required' : 'Write your content here!',
                        heightMin: 500,
                        heightMax: '100vh',
                    }}
                />
