...
const CreateModal = ({ closeModal }) => {
        ...
        <>
            <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center overflow-y-auto">
                <div className="w-[95%] max-w-2xl bg-background shadow-lg border p-2 md:p-4 rounded-md sm:rounded-lg my-6">
                    <div className="py-2 bg-background">
                        <div className="flex items-center justify-between mb-2">
                            <h2 className="text-xl md:text-2xl font-bold text-accent/60">Create Your Literary Piece</h2>
                            <button
                                onClick={() => closeModal(false)}
                                className="rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
                            >
                        ...
                        {/* Content Type Selection - Fixed */}
                        <div>
                            <label className="text-sm md:text-base font-medium text-foreground mb-3 block">Choose your style</label>
                            <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
                                {contentTypes.map((type) => {
                                    const IconComponent = type.icon
                                    return (
                                        <div
                                            key={type.id}
                                            onClick={() => setSelectedType(type.id)}
                                            className={`py-1 md:py-2 bg-accent/10 px-1 md:px-2 bg- rounded-lg border cursor-pointer transition-all duration-200 hover:shadow-sm ${selectedType === type.id
                                                ? "border-accent bg-accent/ shadow-sm"
                                                : "border-border hover:border-accent/50"
                                                }`}
                                        >
                                            <div className="flex items-center space-x-2">
                                                ...
                                                />
                                                <span
                                                    className={`font-medium text-xs md:text-sm ${selectedType === type.id ? "text-accent" : "text-muted-foreground"}`}
                                                >
                                                    {type.label}
                                                </span>
                                            </div>
                    ...
                    <div className="py-2 space-y-4">
                        {/* Title */}
                        <div>
                            <label className="text-sm md:text-base font-medium text-foreground mb-2 block">Title</label>
                            <input
                                placeholder="Give your piece a captivating title..."
                                value={title}
                                onChange={(e) => {
                                ...
                                className={`flex h-10 md:h-12 w-full rounded-md border px-3 py-2 text-sm md:text-base ring-offset-background placeholder:text-muted-foreground transition-all duration-200
    ${titleError
                                        ? "border-red-400 bg-red-50 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-red-400"
                                        : "border-input bg-background focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent"
                                    }
  `}
                        ...
                        {/* Editor - Main scrollable content */}
                        <div>
                            <label className="text-sm md:text-base font-medium text-foreground mb-2 block">Your Content</label>
                            <div ref={scrollContainerRef} className="h-[33vh] overflow-y-auto">
                                <div
                                    className={`min-h-full rounded-lg border p-3 transition-all duration-200 ${contentError ? "border-red-400 bg-red-50" : "border-input focus-within:border-accent"
                                        }`}
                        ...
                        <div className="relative">
                            <label className="text-sm md:text-base font-medium text-foreground mb-2 block">Tags</label>
                            ...
                            <div className="min-h-[44px] w-full rounded-md border border-input bg-background px-3 py-2 mb-3 flex flex-wrap gap-2 items-center">
                                {selectedTags.map((tag) => (
                                    <span
                                        key={tag}
                                        className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-accent/10 text-accent border border-accent/20"
                                    >
                                        #{tag}
                                        <button
                                ...
                                <input
                                    ...
                                    onFocus={handleTagInputFocus}
                                    onBlur={handleTagInputBlur}
                                    className="flex-1 min-w-[120px] bg-transparent outline-none text-sm md:text-base placeholder:text-muted-foreground"
                                />
                            </div>
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                            {categories.map((tag) => (
                                                <button
                                                    ...
                                                    disabled={selectedTags.includes(tag)}
                                                    className={`px-2 py-1 rounded-full text-xs border transition-colors ${selectedTags.includes(tag)
                                                        ? "bg-accent text-accent-foreground border-accent/20 cursor-not-allowed opacity-50"
                                                        : "bg-background hover:bg-accent hover:text-accent-foreground border-border"
                                                        }`}
                                                >
                                                    {tag}
                    ...
                    <div className="py-2 bg-background rounded-b-lg">
                        <div className="flex justify-end space-x-3">
                            <button
                                onClick={handlePublish}
                                className="px-4 py-2 md:py-3 text-sm md:text-base font-medium bg-accent text-accent-foreground rounded-md hover:bg-accent/90 disabled:opacity-50 transition-all duration-200"
                                disabled={!selectedType || titleError || contentError}
                            >
                                {isPublishing ? (
                                    <>
